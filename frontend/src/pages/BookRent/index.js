...
export default function Books(){
    ...
    async function handlePut(e){
        ...
        try{
            const response = await api.put('rents', data);
                        ...
                        <div className="bookBlock">
                            ...
                            <ul>
                                {datas.map(data => (
                                    ...
                                    ? 
                                    <li key={data.id}>
                                        <strong>Livro disponível para aluguel:</strong>
                                        <p>Título do Livro: {(data.rent === 0) ? <div>{data.title}</div> : <div></div>}</p>
                                        <button onClick={handlePut} type="button">Alugar +</button>
                                    </li> 
                                    :
                                    <a></a>
                                ))}
